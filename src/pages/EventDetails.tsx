...
const EventDetails = () => {
  ...
  const [event, setEvent] = useState<{ name: string; details: Event } | null>(
    null
  );
  const [relatedEvents, setRelatedEvents] = useState<
    { name: string; details: Event }[]
  >([]);
      ...
      if (foundEventEntry) {
        ...
        const otherEvents = Object.entries(EVENTS)
          ...
          .map(([otherName, otherDetails]) => {
            ...
            // Location match
            if (
              otherDetails.location.includes(details.location.split(",")[0]) ||
  ...
  const getImageUrl = (imageUrl: string) => {
    ...
    if (imageUrl.startsWith("/")) {
      return `https://afrobeats.party${imageUrl}`;
    }
    return `https://afrobeats.party/${imageUrl}`;
  };

  const DEFAULT_IMAGE = "https://afrobeats.party/AfrobeatsDAOMeta.png";
  const bannerImage = event?.details?.image_url
    ? getImageUrl(event.details.image_url)
    : DEFAULT_IMAGE;
  const isMultiDayEvent = event?.details
    ? event.details.start_date !== event.details.end_date
    : false;
  ...
  const metaDescription = `🎉 Join ${event.name} on ${eventDate} in ${
    event.details.location
  }! ${event.details.event_description.substring(
    0,
    150
  )}... Experience the best Afrobeats culture with ${
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col bg-black text-white pb-[150px] md:pb-[93px]">
        <Header />
        ...
        <main className="flex-1">
          ...
          <div className="w-full relative">
            ...
            <div className="absolute top-20 left-4 z-20">
              <Button
                variant="outline"
                className="bg-black/70 backdrop-blur-sm text-white text-[0.7rem] sm:text-[0.9rem] hover:bg-black/90 border-white/30"
                asChild
              >
                <Link to="/events">
                  <ArrowLeft className="mr-1 sm:mr-2 sm:h-4 h-2  w-2 sm:w-4" />
                  Back to Events
                </Link>
              </Button>
            </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p>
                          ...
                          {isMultiDayEvent &&
                            ` - ${formatDate(event.details.end_date)}`}
                        </p>
                      </div>
                    </div>
              ...
              <div className="lg:col-span-1">
                ...
                {event.details.website && (
                  <div className="bg-gray-900 p-6 rounded-lg shadow-sm mb-8">
                    ...
                    <Button
                      variant="accent"
                      className="w-full bg-[#FFD600] text-black hover:bg-[#FFD600]/90 font-medium"
                      asChild
                    >
                      <a
                        href={event.details.website}
                    ...
                    <Link key={name} to={`/event/${slugify(name)}`}>
                      <Card className="h-full overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-[#008751]/20 hover:border-[#008751]/50 bg-gray-900 border-gray-800 group">
                        ...
                        <CardContent className="p-4">
                          <h3 className="font-bold text-lg mb-2 text-white group-hover:text-[#FFD600] transition-colors duration-300">
                            {name}
                          </h3>
