...
const DancePage = () => {
  ...
  const handleDanceSelect = (dance: any) => {
    ...
    startDance(dance.id);
    navigate(
      `/dance/${selectedGenre === "all" ? dance.genre : selectedGenre}/${
        dance.id
      }`
    );
  };
  ...
  const filterDances = () => {
    ...
    if (selectedGenre === "all") {
      ...
      filteredDances = [
        ...danceCurriculum.afrobeats.map((dance) => ({
          ...dance,
          genre: "afrobeats",
        })),
        ...danceCurriculum.amapiano.map((dance) => ({
          ...dance,
          genre: "amapiano",
        })),
      ];
      ...
      filteredDances = danceCurriculum[
        selectedGenre as keyof typeof danceCurriculum
      ].map((dance) => ({
        ...dance,
        genre: selectedGenre,
      }));
    ...
    if (selectedCountry && selectedCountry !== "all") {
      filteredDances = filteredDances.filter(
        (dance) => dance.origin === selectedCountry
      );
    }
    ...
    if (selectedDifficulty && selectedDifficulty !== "all") {
      filteredDances = filteredDances.filter(
        (dance) => dance.difficulty === selectedDifficulty
      );
    }
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-b from-black to-gray-900 pt-8 px-4 sm:px-6 lg:px-8 transition-opacity duration-500 pb-44 sm:pb-36 ${
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-center text-gray-300 mb-6 sm:mb-8 max-w-3xl mx-auto text-sm sm:text-base">
          Learn authentic Afrobeats and Amapiano dances from beginner to
          advanced levels with our comprehensive curriculum.
        </p>
        ...
        <Card className="bg-[#FFD600] border-[#FFD600] text-black mb-8 shadow-lg">
          <CardHeader>
            <CardTitle className="text-black font-heading">
              Dance Styles
            </CardTitle>
            <CardDescription className="text-black/80 font-medium">
              Choose your style and start learning today
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          {filteredDances.length === 0 ? (
            <div className="col-span-full text-center py-10">
              <p className="text-white text-lg">
                No dances match your current filters.
              </p>
              <Button
                variant="default"
              ...
              >
                  ...
                  <div className="aspect-video w-full bg-gray-900 relative group">
                    <iframe
                      ...
                      src={`https://www.youtube.com/embed/${
                        dance.tutorials[0].link.split("=")[1] ||
                        dance.tutorials[0].link.split("/").pop()
                      }?controls=0`}
                      title={dance.name}
                ...
                <CardContent className="p-3">
                  ...
                  <DanceProgressIndicator
                    danceId={dance.id}
                    totalModules={dance.keyMoves?.length || 5}
                  />

                  {dance.keyMoves && dance.keyMoves.length > 0 && (
                    <div className="mb-3 mt-3">
                      <h4 className="text-sm font-semibold mb-1 text-[#FFD600]">
                        Key Moves:
                      </h4>
