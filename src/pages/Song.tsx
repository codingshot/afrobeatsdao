...
const Song = () => {
  ...
  if (!artist || !song) {
    ...
    return (
      <>
        <Helmet>
          <title>
            Song Not Found | Afrobeats.party - African Music & Culture
          </title>
          <meta
            name="description"
            content="Song not found on Afrobeats.party - Discover the best African music, artists, and Afrobeats culture. Join the global Afrobeats community."
          />
          <meta
            property="og:title"
            content="Song Not Found | Afrobeats.party"
          />
          <meta
            property="og:description"
            content="Discover the best African music, artists, and Afrobeats culture on Afrobeats.party"
          />
          <meta
            property="og:image"
            content="https://afrobeats.party/AfrobeatsDAOMeta.png"
          />
  ...
  const metaTitle =
    ...
    " by " +
    safeArtistName +
    (safeArtistCountry ? " - " + safeArtistCountry : "") +
    " | Afrobeats.party";
  const metaDescription =
    ...
    " on Afrobeats.party. " +
    (safeArtistCountry
      ? "Discover this " + safeArtistCountry + " artist's "
      : "Explore ") +
    safeArtistGenre +
    " music and join the global African music community. Stream now!";
  const canonicalUrl =
    "https://afrobeats.party/music/artist/" + safeArtistId + "/" + safeSongSlug;

  // Use YouTube thumbnail as primary OG image, ensure it's a full URL
  const ogImage = videoId
    ? "https://img.youtube.com/vi/" + videoId + "/maxresdefault.jpg"
    : artist.image
    ? "https://afrobeats.party" + artist.image
    ...
    try {
      const data = {
        ...
        isPartOf: {
          ...
          description:
            "Global platform for African music and Afrobeats culture",
        },
        potentialAction: {
          "@type": "ListenAction",
  ...
  const otherArtistsSongs = ARTISTS.filter((a) => a.id !== artist.id)
    .flatMap((a) =>
      a.top_songs.map((s) => ({ ...s, artistName: a.name, artistId: a.id }))
    )
    ...
    <>
      <Helmet>
        ...
        <meta
          ...
          content={"https://afrobeats.party/music/artist/" + safeArtistId}
        />
        ...
        <meta
          name="geo.region"
          content={safeString(safeArtistCountry) || undefined}
        />
        <meta
          name="geo.placename"
          content={safeString(safeArtistCountry) || undefined}
      ...
      <div className="min-h-screen bg-gray-50 pt-8 pb-48 md:pb-36">
        <div className="container mx-auto px-4 max-w-6xl">
          {/* Breadcrumb */}
          <div className="mb-6 text-sm text-gray-600">
            <Link to="/music" className="hover:text-[#008751]">
              Music
            </Link>
                    ...
                    {otherArtistsSongs.map((relatedSong, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <Link
                            to={`/music/artist/${
                              relatedSong.artistId
                            }/${slugify(relatedSong.title)}`}
                        ...
                        <div className="flex items-center gap-1">
                          <Button
                            ...
                            onClick={() => {
                              const relatedVideoId = getVideoId(
                                relatedSong.youtube
                              );
                              playNow({
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {relatedSongs.slice(0, 5).map((relatedSong, index) => (
                        ...
                        >
                          <Link
                            to={`/music/artist/${artist.id}/${slugify(
                              relatedSong.title
                            )}`}
