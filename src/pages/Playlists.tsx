...
const Playlists = () => {
  ...
  const handleSort = () => {
    if (sortMode === "default") setSortMode("asc");else if (sortMode === "asc") setSortMode("desc");else setSortMode("default");
  };
  const filteredPlaylists = useMemo(() => {
    let filtered = PLAYLISTS.filter(playlist => {
      const matchesSearch = playlist.title.toLowerCase().includes(searchQuery.toLowerCase()) || playlist.description.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const filteredArtists = useMemo(() => {
    return TRENDING_ARTISTS.filter(artist => artist.name.toLowerCase().includes(searchQuery.toLowerCase()) || artist.country.toLowerCase().includes(searchQuery.toLowerCase()) || artist.popular_song.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [searchQuery]);
      ...
      <main className="container mx-auto px-4 py-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 w-full">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search playlists or artists..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 bg-white text-black w-full" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <div className={`col-span-1 md:col-span-2 lg:col-span-3 ${queueVisible ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
            <Tabs defaultValue="playlists" className="w-full">
              ...
              <TabsContent value="playlists" className="mt-6 space-y-4 w-full">
                <AnimatePresence>
                  {filteredPlaylists.length > 0 ? filteredPlaylists.map(playlist => <motion.div key={playlist.id} initial={{
                  opacity: 0,
                  y: 10
                }} animate={{
                  opacity: 1,
                  ...
                  duration: 0.3
                        ...
                        <div className="h-20 w-20 flex-shrink-0 overflow-hidden rounded-md">
                          <img src={playlist.image} alt={playlist.title} className="h-full w-full object-cover" onError={handleImageError} />
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          {playlist.platform === 'youtube' ? <>
                              <Button variant="ghost" size="icon" onClick={() => playNow({
                        id: playlist.id,
                        ...
                        artist: "Playlist"
                              ...
                              <Button variant="ghost" size="icon" onClick={() => handleAddToQueue(playlist)} className="text-[#008751] hover:text-[#008751]/90 hover:bg-[#008751]/10">
                                <List className="h-5 w-5" />
                              </Button>
                            </> : <Button variant="ghost" size="icon" asChild className="text-[#008751] hover:text-[#008751]/90 hover:bg-[#008751]/10">
              ...
              <TabsContent value="artists" className="mt-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {filteredArtists.map(artist => <motion.div key={artist.id} initial={{
                  opacity: 0,
                  scale: 0.95
                }} animate={{
                  opacity: 1,
                  ...
                  duration: 0.3
                      ...
                      <div className="h-32 w-32 overflow-hidden rounded-full mb-4">
                        <img src={artist.image} alt={artist.name} className="h-full w-full object-cover" onError={handleImageError} />
                      </div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        {artist.youtube_id && <Button onClick={() => playArtistSong(artist)} size="sm" className="bg-[#008751] hover:bg-[#008751]/90">
                            <Play className="mr-2 h-4 w-4" />
                            Play Track
                          </Button>}
                        
                        {artist.spotify_url && <Button variant="outline" size="sm" asChild>
          ...
          {queueVisible && <motion.div initial={{
          opacity: 0,
          x: 20
        }} animate={{
          opacity: 1,
              ...
              <Card className="h-full border bg-white">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => setShowPlayedSongs(!showPlayedSongs)} className={`${showPlayedSongs ? 'bg-primary/10' : ''} text-black`}>
                  ...
                  <ScrollArea className="h-[500px] pr-4">
                    {filteredQueue.length > 0 ? <DragDropContext onDragEnd={handleDragEnd}>
                        ...
                        return <Draggable key={`${song.id}-${index}`} draggableId={`${song.id}-${index}`} index={index}>
                                    {provided => <div ref={provided.innerRef} {...provided.draggableProps} className="flex items-center gap-3 p-2 rounded-md hover:bg-accent/10 group">
                                        ...
                                        <div className="relative flex-shrink-0 w-16 h-12 rounded-md overflow-hidden">
                                          <img src={getVideoThumbnail(videoId)} alt={song.title || "Video thumbnail"} className="w-full h-full object-cover" onError={e => {
                                e.currentTarget.src = "/AfrobeatsDAOMeta.png";
                              }} />
                                          <Button variant="ghost" size="icon" className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-black/50 hover:bg-black/70 text-white" onClick={() => playNow(song)}>
                                        ...
                                        {playedSongs.has(song.id) && <Badge variant="outline" className="ml-auto">
                                            Played
                                          </Badge>}
                                      </div>}
                                  </Draggable>;
                      ...
                      </DragDropContext> : <div className="text-center py-8 text-muted-foreground">
