    ...
    if (clubs.length > 0) {
      ...
      if (validClubs.length > 0) {
        const bounds = L.latLngBounds(
          validClubs.map(
            (club) =>
              [club.coordinates![1], club.coordinates![0]] as L.LatLngExpression
          )
        );
...
const ClubsMapView: React.FC<ClubsMapViewProps> = ({
  clubs,
  filters,
  onSelectClub,
}) => {
  ...
  return (
    <div className="flex flex-col gap-4">
                  ...
                  <Card className="border-0 shadow-none">
                    <CardContent className="p-2">
                      ...
                      <div className="space-y-1 text-sm">
                        <p>
                          <span className="font-medium">Type:</span> {club.type}
                        </p>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {club.music.split(",").map((genre, i) => (
                              <Badge
                                key={i}
                                variant="outline"
                                className="text-xs bg-[#F97316]/10"
                              >
                                {genre.trim()}
                              </Badge>
                            ))}
      ...
      <div className="w-full px-3 relative h-fit">
        <Carousel
          opts={{
            align: "start",
            loop: true,
        ...
        >
          <CarouselContent>
            {clubs.map((club, index) => (
              <CarouselItem
                key={index}
                className="md:basis-1/3 lg:basis-1/4 py-1"
              >
                <Card
                  className={`py-2 mx-1 h-full transition-all duration-200 cursor-pointer ${
                ...
                >
                  <CardContent className="space-y-2">
                    ...
                    <div className="text-sm text-black">{club.hours}</div>
